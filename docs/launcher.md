...
{% shot "launcher-window", "the full Launcher window: search field on top, the pin list on the left with one row selected, the rich preview card on the right, group chips under the search field, and the hint bar along the bottom.", "window" %}
...
{% shot "window-style-segmented-control", "close-up of the Window Style segmented control in Settings → General with Popover and Launcher side by side, Launcher selected.", "dialog" %}
...
{% shot "launcher-group-autocomplete", "close-up of the search field with a group name half-typed and the inline autocomplete ghost completing it, group chips visible below.", "hud" %}
...
{% shot "launcher-power-peek", "the Launcher in Power-Peek mode (⌘⌥ held): each row showing its raw URL or path and the numbered 1–9 chips on the first rows.", "window" %}
...
{% shot "cards-floating-desktop", "two or three Cards floating over a desktop — one web page, one file — while another app is frontmost, showing they stay on top.", "dialog" %}
